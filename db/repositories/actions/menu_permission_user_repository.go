...
func (repository MenuPermissionUserRepository) Browse(userID string) (data []models.MenuPermissionUser, err error) {
	statement := `select mpu.*, mp."permission",m."id",m."name" from "menu_permission_users" mpu 
                  ...
                  where mpu."user_id"=$1 and mpu."deleted_at" is null`
...
func (MenuPermissionUserRepository) Add(userID, menuPermissionID, createdAt, updatedAt string, tx *sql.Tx) (err error) {
	statement := `insert into "menu_permission_users" ("user_id","menu_permission_id","created_at","updated_at") values($1,$2,$3,$4)`
...
func (MenuPermissionUserRepository) Delete(userID, menuPermissionID, updatedAt, deletedAt string, tx *sql.Tx) (err error) {
	statement := `update "menu_permission_users" set "updated_at"=$1,"deleted_at"=$2 where "user_id"=$3 and "menu_permission_id"=$4`
