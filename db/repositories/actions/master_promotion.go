...
func (repository MasterPromotionRepository) Browse(filters map[string]interface{}, order, sort string, limit, offset int) (data []models.MasterPromotion, count int, err error) {
	...
	statement := masterPromotionSelect+` from "promotion_packages" where "deleted_at" is null `+filterStatement+` order by ` + order + ` ` + sort + ` limit $1 offset $2`
	...
	statement = `select count("id") from master_promotion_helper where "deleted_at" is null`+filterStatement
...
func (repository MasterPromotionRepository) BrowseAll() (data []models.MasterPromotion,err error){
	statement := `select * from master_promotion_helper`
...
func (repository MasterPromotionRepository) Edit(input viewmodel.MasterPromotionVm) (res string, err error) {
	statement := `update master_promotion_helper set name=$1, "slug"=$2, "updated_at"=$3, "is_active"=$4 where "id"=$5 returning "id"`
...
func (repository MasterPromotionRepository) Add(input viewmodel.MasterPromotionVm) (res string, err error) {
	statement := `insert into master_promotion_helper (name,"slug","created_at","updated_at","is_active") values($1,$2,$3,$4,$5) returning "id"`
...
func (repository MasterPromotionRepository) Delete(ID, updatedAt, deletedAt string) (res string, err error) {
	statement := `update master_promotion_helper set "updated_at"=$1, "deleted_at"=$2 where "id"=$3 returning "id"`
